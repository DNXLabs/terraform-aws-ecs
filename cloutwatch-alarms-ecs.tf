resource "aws_cloudwatch_metric_alarm" "ecs_high_memory" {
  ...
  alarm_name                = "${var.ignore_iam_account_alias[0] ? "ecs-${var.name}-high-memory" : data.aws_iam_account_alias.current[0]}-ecs-${var.name}-high-memory"
...
resource "aws_cloudwatch_metric_alarm" "ecs_high_cpu" {
  ...
  alarm_name                = "${var.ignore_iam_account_alias[0] ? "ecs-${var.name}-high-cpu" : data.aws_iam_account_alias.current[0]}-ecs-${var.name}-high-cpu"
  comparison_operator       = "GreaterThanOrEqualToThreshold"
  evaluation_periods        = "3"
